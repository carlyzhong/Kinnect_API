...
const getCodingArticles = async () => {
    const result = await db.query(`SELECT * FROM articles WHERE topic = 'coding';`);
    console.log('>>>>>>>Get all of the articles where the topic is coding:<<<<<<<< \n', result.rows)
    return result.rows
}

const getNegativeVoteCommants = async () => {
    const result = await db.query(`SELECT * FROM comments WHERE votes < 0 ORDER BY votes ASC;`);
...
const getUserArticles = async (username) => {
    const result = await db.query(`SELECT * FROM articles WHERE author = '${username}';`);
    console.log('>>>>>>>Get all of the topics:<<<<<<<< \n', result.rows)
    return result.rows
}

const getHighVoteCommants = async () => {
    const result = await db.query('SELECT * FROM comments WHERE votes >= 10 ORDER BY votes DESC;');
